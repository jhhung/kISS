...
calculate_new_rank_head(const std::ranges::random_access_range auto& sa,
  ...
  auto n_ = (size_type)sa.size() - 1;
  auto n2 = (size_type)rank.size() - 1;
  std::copy(std::execution::par, std::begin(rank), std::begin(rank) + (n2 + 1),
            std::begin(buf));
...
radix_sort(std::ranges::random_access_range auto& sa,
  ...
  // for (auto i : is_head) std::cout << (int)i << ' ';
  // std::cout << std::endl;
  compact(sa, rank, buf, is_head, index_offset);
  // std::cout << "<<\n";
  // for (auto i : sa) std::cout << i << ' ';
  // std::cout << std::endl;
  // for (auto i : rank) std::cout << i << ' ';
...
prefix_doubling(const std::ranges::random_access_range auto& sa,
  ...
  // prefix doubling
  auto sa_dup = sa;
  for (uint64_t i = 1; i * init_length < sort_len; i *= 2) {
...
#pragma omp critical
  ...
  for (auto i = size_type{}; i < NUM_THREADS; i++) {
    if (have_lms[i]) {
      if (have_last_block_lms) {
        len[i - 1] += (first_lms[i] - last_block_lms + compress_block_length)
                      / compress_block_length;
      }
  ...
  // do mapping: n2 index -> normal index (+ boolean array to know whether each
  // index is valid)
...
#pragma omp parallel for num_threads(NUM_THREADS)
  for (auto tid = size_type{}; tid < NUM_THREADS; tid++) {
    ...
    for (auto i = L; i < R; i++) {
      if (valid[sa[i]])
        sorted_lms[local_count_sorted_lms++] = buf[sa[i]];
    }
  }
}
#undef MASK
