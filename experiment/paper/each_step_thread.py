    ...
    for algo in algos:
        for k in ks:
            ...
            for _, x in times:
                ...
                file_name = ('./sa_thread/log_%s_%s_%d_%d.txt' % (algo_, d, k, x))
                m1 = {}
...
for d in ['dna', 'protein', 'hs37d5', 'mouse', 'zebrafish']:
    ...
    ax.set_yticks(np.arange(0, r[d], (50 if d != 'dna' and d != 'protein' else 2)),
     np.arange(0, r[d], (50 if d != 'dna' and d != 'protein' else 2)), fontsize=28)
plt.savefig('./sa/total_each_step_thread.png', dpi=300)