...
auto run(
  ...
  if (is_general) {
    ...
    biovoltron::kiss::kiss1_suffix_array(S_input, SA, k, num_threads);

    SPDLOG_DEBUG("kiss-1 elapsed {} {}", sw, S_input.size());
    SPDLOG_DEBUG("Memory usage {}", getPeakRSS());

  } else {
    auto sw = spdlog::stopwatch{};

    biovoltron::kiss::kiss1_suffix_array_dna(S_input, SA, k, num_threads);

    SPDLOG_DEBUG("kiss-1 elapsed {}", sw);
    SPDLOG_DEBUG("Memory usage {}", getPeakRSS());
